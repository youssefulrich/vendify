...
export default function ProduitsPage() {
  ...
  const [form, setForm] = useState({
    ...
    stock: '', stock_alerte: '5', actif: true, photo_url: ''
  })
  ...
  function openNew() {
    setEditProduct(null)
    setForm({ nom: '', description: '', prix_achat: '', prix_vente: '', stock: '', stock_alerte: '5', actif: true, photo_url: '' })
    setPreviewUrl('')
    setPhotoFile(null)
    setShowForm(true)
  }
  ...
  function openEdit(p: any) {
    ...
    setForm({
      ...
      prix_vente: String(p.prix_vente), stock: String(p.stock),
      stock_alerte: String(p.stock_alerte), actif: p.actif, photo_url: p.photo_url || ''
    })
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto' }}>
      <style>{`
        ...
        .prod-price { font-size: 16px; font-weight: 800; color: #f5a623; margin-bottom: 10px; }
        .prod-actions { display: flex; gap: 8px; }
        .btn-edit { flex: 1; padding: 8px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; color: #c8cdd8; font-size: 12px; cursor: pointer; transition: all 0.15s; }
        .btn-edit:hover { background: rgba(255,255,255,0.08); }
        .btn-del { padding: 8px 12px; background: rgba(255,80,80,0.08); border: 1px solid rgba(255,80,80,0.15); border-radius: 8px; color: #ff5e5e; font-size: 12px; cursor: pointer; transition: all 0.15s; }
      ...
      {!isPremium && (
        <div style={{ background: 'rgba(245,166,35,0.06)', border: '1px solid rgba(245,166,35,0.15)', borderRadius: 12, padding: '12px 16px', marginBottom: 20, fontSize: 12, color: '#f5a623', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      {products.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '80px 20px', color: '#717a8f' }}>
          ...
          <div style={{ fontSize: 13 }}>Ajoutez votre premier produit</div>
        </div>
      ) : (
          ...
          {products.map((p: any) => (
            <div key={p.id} className="prod-card">
              ...
              <div className="prod-body">
                ...
                <div className="prod-price">{formatCFA(p.prix_vente)}</div>
        ...
        <div className="modal-bg" onClick={() => setShowForm(false)}>
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
              <h2 style={{ fontSize: 18, fontWeight: 800 }}>{editProduct ? 'Modifier le produit' : 'Nouveau produit'}</h2>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
              <button onClick={() => setShowForm(false)} style={{ flex: 1, padding: '12px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, color: '#717a8f', fontSize: 14, cursor: 'pointer' }}>
                Annuler
              </button>
              <button onClick={handleSave} disabled={saving || uploading} style={{
                flex: 2, padding: '12px', background: 'linear-gradient(135deg, #f5a623, #ffcc6b)',
                border: 'none', borderRadius: 12, color: '#000', fontSize: 14, fontWeight: 700, cursor: 'pointer'
              }}>
