...
export default function LivreurDashboardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadAll() {
    ...
    // On ne joint pas orders car les livraisons publiques n'en ont pas
    const { data: dispo, error: dispoErr } = await supabase
      .from('deliveries')
      .select('*')
      .eq('status', 'pending')
      .eq('ville', drv.ville)
    ...
    if (dispoErr) console.error('Erreur dispo:', dispoErr)
    ...
    if (coursesErr) console.error('Erreur courses:', coursesErr)

    setDisponibles(dispo || [])
    setMesCourses(courses || [])
    setLoading(false)
    console.log('Chargement terminé — dispo:', (dispo||[]).length, 'courses:', (courses||[]).length)
  }
