  ...
  useEffect(() => {
    ...
    async function load() {
      const { data: p } = await supabase.from('profiles')
        .select('id,shop_name,full_name,phone,pays,plan').eq('shop_slug', slug).single()
    ...
    <>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .panel-price { font-family: 'Playfair Display', serif; font-size: 30px; font-weight: 900; color: #f5a623; margin-bottom: 8px; }
      ...
      {/* SUCCESS + NOTIFICATION WHATSAPP VENDEUR */}
      {orderResult && (
        <div className="success-overlay">
          <div className="success-box">
            ...
            <p style={{ fontSize: 13, color: '#666', lineHeight: 1.7, marginBottom: 24 }}>
              Votre commande a bien été reçue par <strong style={{ color: '#f0ede8' }}>{profile.shop_name}</strong>. Cliquez ci-dessous pour notifier le vendeur sur WhatsApp.
            </p>

            {/* Bouton notifier vendeur */}
            {orderResult.vendor_phone && (() => {
              const vendorPhone = orderResult.vendor_phone.replace(/\D/g, '')
              const msg = `🛍 *Nouvelle commande !*\n\n` +
                ...
                `Via Vendify 🏪`
              return (
                <a href={`https://wa.me/${vendorPhone}?text=${encodeURIComponent(msg)}`}
                  target="_blank" rel="noopener noreferrer"
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, background: '#25d366', color: '#fff', borderRadius: 12, padding: '13px', fontSize: 14, fontWeight: 700, textDecoration: 'none', marginBottom: 10, width: '100%' }}>
            ...
            <button onClick={() => setOrderResult(null)}
              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#f0ede8', borderRadius: 12, padding: '12px', fontSize: 13, cursor: 'pointer', width: '100%' }}>
